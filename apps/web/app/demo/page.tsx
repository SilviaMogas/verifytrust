...
export default function Demo() {
 ...
 return <main className="shell" key={reset}>
   ...
   <section className="hero">
     <div className="eyebrow">NOVA GOODS · DEMO MERCHANT</div>
     <h1>Proof, not<br /><span className="accent">paperwork.</span></h1>
     <p>Your purchase stays private. The proof becomes verifiable.</p>
   </section>
   ...
   <section className="section"><div className="card">
     <div className="label">STEP {step} / 07</div>
     <h2>{steps[step-1]}</h2>
     ...
     {step===3&&!proof&&<><p>PRIVATE PURCHASE DATA ↓ LOCAL ZK PROVER ↓ PUBLIC PROOF</p><button className="button" disabled={busy} onClick={prove}>{busy?"PROVING…":"GENERATE PRIVATE PROOF"}</button></>}
     {step===3&&proof&&<><p className="status">{message || "VERIFIED LOCALLY"}</p><p className="mono">PUBLIC INPUTS<br />{proof.publicInputs.join("\n")}</p><p className="mono">PROOF SIZE · {Math.round((proof.proof.length-2)/2)} BYTES</p><button className="button" onClick={()=>setStep(4)}>WRITE REVIEW</button></>}
     {step===4&&<><textarea value={review} onChange={e=>setReview(e.target.value)} /><div className="actions">{[1,2,3,4,5].map(value=><button className={value===rating?"button":"button secondary"} key={value} onClick={()=>setRating(value)}>{value} ★</button>)}</div><button className="button" onClick={makeCommitment}>COMPUTE REVIEW COMMITMENT</button></>}
     ...
     {step===6&&<><div className="grid"><div className="card status">VERIFIED PURCHASE</div><div className="card status">PRIVACY PROTECTED</div><div className="card status">ETHEREUM VERIFIED</div></div><p className="mono">NETWORK · {process.env.NEXT_PUBLIC_EXPLORER_URL?"SEPOLIA":"ANVIL"}<br />REGISTRY · {registry}<br />PROOF STATUS · VERIFIED<br />NULLIFIER STATUS · USED<br />VERIFICATION TRANSACTION · {txHash}</p><button className="button" onClick={checkDuplicate}>ATTEMPT DUPLICATE REVIEW</button></>}
     {step===7&&<><p className="status">{message || "CLIENT CHECK · NULLIFIER USED"}</p><button className="button" disabled={busy} onClick={()=>submit(true)}>RESUBMIT SAME PROOF</button><p>{message}</p></>}
