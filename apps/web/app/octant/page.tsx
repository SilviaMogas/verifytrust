...
const sections: [string, string][] = [
  ...
  ["PROJECT HISTORY", "2023 — Verify Trust founded around a blockchain-backed Proof of Review protocol (Proof of Review Protocol, Verify Trust Hub prototype, digital-identity concept, Web2.5/Web3 onboarding).\n\n2023 — Selected as Best Idea at the CEIN Awards (Navarra, Spain).\n\n2023 — Selected by ACCIÓ to represent Catalonia at Web Summit Lisbon 2023.\n\nIncubated by The LIST (Luxembourg).\n\n2026 — Protocol rebuild: privacy-preserving ZK architecture (this repository)."],
...
export default function Octant() {
  ...
  return <main className="shell"><Nav /><section className="hero"><div className="eyebrow">VERIFY TRUST</div><h1>Privacy-Preserving<br />Proof of Review</h1><p className="accent">PROVE THE PURCHASE. PROTECT THE PERSON.</p><p>Verify Trust originally focused on proving that a review came from a real customer. The 2026 protocol rebuild focuses on solving the harder problem: proving that claim without identifying the customer.</p></section>{sections.map(([heading,body])=><section className="section" id={heading.toLowerCase().replaceAll(" ","-")} key={heading}><h2 className="label">{heading}</h2><p style={{whiteSpace:"pre-line",maxWidth:800}}>{body}</p>{heading==="LIVE DEMO"&&<Link className="button" href="/demo">OPEN LIVE DEMO</Link>}{heading==="CONTRACTS"&&<div className="mono">Anvil · 31337<br />HonkVerifier · {local.honkVerifier}<br />ProofOfReviewVerifier · {local.proofVerifier}<br />IssuerRegistry · {local.issuerRegistry}<br />VerifyTrustRegistry · {local.verifyTrustRegistry}</div>}{heading==="OPEN SOURCE"&&<div className="actions">{docs.map(doc=><a key={doc} href={`https://github.com/SilviaMogas/verifytrust/blob/main/docs/${doc}.md`}>{`docs/${doc}.md`}</a>)}</div>}</section>)}<Footer /></main>;
}
